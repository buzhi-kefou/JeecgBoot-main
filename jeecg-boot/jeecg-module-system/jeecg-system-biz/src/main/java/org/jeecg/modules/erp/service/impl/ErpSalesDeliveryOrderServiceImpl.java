...
@Slf4j
@Service
public class ErpSalesDeliveryOrderServiceImpl extends ServiceImpl<ErpSalesDeliveryOrderEntityMapper, ErpSalesDeliveryOrderEntity>
        implements IErpSalesDeliveryOrderService {
    ...
    @Override
    public List<ErpSalesDeliveryOrderEntity> queryByDate(String beginDateStr, String endDateStr) {
        String filterString = "";
        ...
        detailDto.setFieldKeys(new ErpSalesDeliveryOrderEntity());
        detailDto.setFilterString(filterString);
        detailDto.setOrderString("FModifyDate desc");
        detailDto.setFormId("SAL_OUTSTOCK");

        QueryDto queryDto = new QueryDto();
        queryDto.setParameters(List.of(detailDto));

        List<ErpSalesDeliveryOrderEntity> request = erpRequestService.request(queryDto, ErpSalesDeliveryOrderEntity.class);
        transactionTemplate.execute(status -> {
            saveOrUpdateSalesDeliveryOrders(request);
            return null;
        });
    ...
    private void saveOrUpdateSalesDeliveryOrders(List<ErpSalesDeliveryOrderEntity> request) {
        List<ErpSalesDeliveryOrderEntity> insertList = new ArrayList<>();
        List<ErpSalesDeliveryOrderEntity> updateList = new ArrayList<>();
        if (CollUtil.isNotEmpty(request)) {
            for (ErpSalesDeliveryOrderEntity entity : request) {
                ErpSalesDeliveryOrderEntity byId = baseMapper.selectById(entity.getFid());
                if (byId == null) {
